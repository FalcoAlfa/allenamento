...
**Prossimo passo: nessuna fase aperta, e nessuna domanda aperta.** La §11.2 (se il test di riferimento debba diventare una terza modalità *Test* in home) è chiusa con un sì il 10/09/2026: vedi la fase 6. Il testo della §11.2 è rimasto com'era, si legge con questa riga accanto. La revisione delle schede del 05/09/2026 segue il §9, non è una fase.
...
| 5 · Riepilogo esteso | ✅ 06/09/2026 | `riepilogo.py` legge anche `sedute.jsonl` e stampa una sezione **STORICO**: durata media per seduta, gli esercizi saltati più spesso, da quante misure ogni numero chiave non si muove. Nessuno script nuovo: tutto dentro il file che c'era già, con i suoi assert.<br><br>**Fatta con una seduta in meno della soglia del §8**, su sua richiesta. La soglia serviva a non scrivere codice per un file vuoto, e a nove sedute il file non è vuoto. Il §8 resta com'è scritto: è questa riga a dire cosa è successo davvero.<br><br>**Quattro cose che il piano non prevedeva.** **(a)** Le medie sono **per sigla di seduta** (M1, M2, M3, A, B), non per `tipo`: mattutina contro forza dà due medie che già si sanno, A contro B è la differenza che si guarda davvero. **(b)** Nel JSONL gli esercizi saltati sono **sigle nude**, ma il §9 dice che la chiave vera è sezione + sigla: `F1` è un esercizio in A e un altro in B. Il riepilogo risolve i nomi leggendo i titoli delle schede e, quando la sigla è ambigua, li mostra **tutti e due** (`Mountain climber / Burpee`) invece di sceglierne uno a caso. Chiuderla per davvero vorrebbe dire cambiare cosa scrive l'app nel JSONL: non l'ho fatto, è una decisione tua. **(c)** *"Da quante **sedute** un numero non si muove"* non è calcolabile: la mattutina non registra quasi mai numeri chiave, quindi contare le sedute gonfierebbe ogni stallo. Conta le **misure**, e tiene distinti tre casi — *fermo da N misure*, *cambiato il …*, *unica misura*: un numero visto una volta sola non è un numero fermo, e leggerlo come tale porterebbe a cambiare una scheda per niente. **Trappola che resta aperta**: le misure di una stessa voce arrivano da sedute diverse — il dead hang lo registra sia R5 nella forza sia il mattutino — e finiscono in una serie sola, come se fossero lo stesso test. Non lo sono. Per distinguerle il posto è `chiave` nel JSONL, cioè l'app, non il riepilogo. **(d)** Da qui `riepilogo.py` legge anche `allenamento-forza.md` e `routine-mattutina.md`; se mancano non si rompe, stampa la sigla. L'autotest però gira su **schede finte**, apposta: la lezione della fase 2 è che un cambio alle schede può rompere un assert che non c'entra niente.<br><br>**Verifica fatta**: `python diario/riepilogo.py` — l'autotest interno gira prima della stampa e copre durate, saltati e numeri fermi — e le tre sezioni nuove escono. Con poche sedute dentro la classifica dei saltati è ancora piatta: si riempie di senso col tempo, non con altro codice. |
| 6 · Modalità Test | ✅ 10/09/2026 | Chiude la **§11.2** con un sì. Terza modalità in home: gli otto numeri della tabella *Test di riferimento* del diario, nel loro ordine, più la presa delle trazioni e una nota. Niente passi, niente cronometro. L'export produce la riga già pronta per quella tabella.<br><br>**La ragione della decisione non è quella che il piano immaginava, e cambia cosa si è costruito.** La §11.2 diceva *«se dopo un paio di giri risulta scomodo»*, e con quel motivo la modalità non valeva il codice: sono due minuti, tredici volte l'anno. Il motivo vero è un altro, e l'ha detto lui: **si dimentica di fare il test**. Quindi il pezzo che risolve il problema non è il modulo di inserimento, è il **promemoria in home**: una card quando sono passate 4 settimane dall'ultimo test registrato, una riga piccola col conto alla rovescia quando non lo sono. Senza nessun test registrato la card è accesa, che al primo giro è la risposta giusta.<br><br>**Contraddice il §10?** No, e va scritto perché a leggerlo sembra di sì. Il §10 esclude *«notifiche e promemoria»* con la ragione *«non ho un problema di memoria, ho un problema di dati»*, e parla della routine di tutti i giorni, che si fa appena sveglio e la sera e non si scorda. Un test ogni 4 settimane si scorda. E qui non c'è nessuna notifica: è testo dentro una schermata che apre da solo due volte al giorno, niente permessi e niente push. Il §10 resta com'è scritto.<br><br>**Due cose escluse apposta.** **(a)** Il test **non entra in `sedute.jsonl`**: la tabella dei test è la sua unica casa, due posti per lo stesso dato prima o poi divergono, e così `riepilogo.py` non si tocca. **(b)** Niente cronometro nella modalità Test: i secondi li misura col telefono come ha sempre fatto, e replicare il timer fuori dalla seduta è codice nuovo per una cosa che non è mai stata un problema.<br><br>**Cose che il piano non prevedeva.** `CHIAVE` ha adesso **due usi**: i campi di fine seduta (il campo `k`, le sigle delle schede) e le **colonne della tabella dei test**, cioè l'ordine dell'array. Le **trazioni** ci sono entrate con `k` vuoto: non stanno in nessuna seduta, quindi niente campo a fine seduta, ma la colonna serve. Se qualcuno riordina l'array i numeri finiscono nella colonna sbagliata e non se ne accorge nessuno, quindi c'è un assert che lo prende. `rigaLog` adesso **scarta da sola** i record di tipo test: test e seduta nello stesso giorno sono lo stesso giorno, e i numeri del test non devono entrare nella riga di log. La **nota del giorno** invece li unisce, ed è giusto così. Le due colonne che portano già l'unità nell'intestazione (*Negativa (s)*, *Mani-muro (cm)*) non la ripetono nella cella: lo detta la tabella che c'era, non il codice.<br><br>**Verifica fatta**: `?test=1` in Chrome headless dice *test ok*, con gli assert nuovi su riga del test, numero e ordine delle colonne, esclusione dal log e dal JSONL, conto delle 4 settimane. Poi il giro intero provato fuori dal browser su `index.html` appena costruito: home nei tre stati (mai fatto, in ritardo, fra N giorni), schermata, salvataggio, export da solo e insieme a una seduta dello stesso giorno, confronto coi valori del test precedente, salvataggio a campi vuoti rifiutato. **Un assert nuovo era sbagliato e il codice aveva ragione**: la colonna *Crow* è in secondi come *Dead hang*, e la riga che c'era nel diario non lo lasciava vedere. |
